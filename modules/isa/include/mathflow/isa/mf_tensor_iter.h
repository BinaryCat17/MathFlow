...
static inline void* mf_tensor_iter_get_at_linear(const mf_tensor* t, size_t linear_index) {
    if (mf_tensor_is_contiguous(t)) {
        return (uint8_t*)mf_tensor_data(t) + linear_index * mf_dtype_size(t->info.dtype);
    }
    ...
    for (int i = t->info.ndim - 1; i >= 0; --i) {
        indices[i] = count % t->info.shape[i];
        count /= t->info.shape[i];
    }
    return (uint8_t*)mf_tensor_data(t) + mf_tensor_get_offset(t, indices) * mf_dtype_size(t->info.dtype);
}
