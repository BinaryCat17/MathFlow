...
static void _setup_resource_inst(mf_resource_inst* res, const char* name, const char* provider, mf_dtype dtype, const int32_t* shape, uint8_t ndim, uint8_t flags, mf_arena* arena) {
    res->name = mf_arena_strdup(arena, name);
...
static void analyze_transience(mf_engine* engine) {
    for (u32 r_idx = 0; r_idx < engine->resource_count; ++r_idx) {
        ...
        if (!read_before_write && write_happened) res->flags |= MF_RESOURCE_FLAG_TRANSIENT;
    }
}
...
static void apply_initial_data(mf_engine* engine) {
    for (u32 k = 0; k < engine->kernel_count; ++k) {
        mf_kernel_inst* ker = &engine->kernels[k];
        for (u32 b = 0; b < ker->binding_count; ++b) {
...
void mf_engine_bind_cartridge(mf_engine* engine, mf_program** programs, uint32_t count) {
    ...
    // 2. Init Kernels
    engine->kernels = MF_ARENA_PUSH(&engine->arena, mf_kernel_inst, count);
    ...
    for (u32 k = 0; k < count; ++k) {
        ...
        for (u32 s = 0; s < prog->meta.symbol_count; ++s) {
            ...
            if (r_idx != -1) {
                mf_kernel_binding* kb = &inst->bindings[inst->binding_count++];
...
void mf_engine_bind_pipeline(mf_engine* engine, const mf_pipeline_desc* pipe, mf_program** programs) {
    ...
    for (u32 i = 0; i < pipe->kernel_count; ++i) {
        mf_pipeline_kernel* d = &pipe->kernels[i];
        mf_kernel_inst* k = &engine->kernels[i];
        k->id = mf_arena_strdup(&engine->arena, d->id);
        k->id_hash = mf_fnv1a_hash(k->id);
        ...
        for (u32 b = 0; b < d->binding_count; ++b) {
            ...
            if (s_idx != -1 && r_idx != -1) {
                mf_kernel_binding* kb = &k->bindings[k->binding_count++];
