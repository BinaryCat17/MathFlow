...
- [x] **Static Binding Validation:** Реализовать проверку совместимости графа и ресурсов ПЕРЕД запуском:
...
- [x] **Host-Compiler Schema Sync:** Автоматизировать передачу метаданных о глобальных ресурсах из `.mfapp` в контекст компилятора для корректного вывода типов.
- [x] **Thread-Local Safety:** Изолировать временные структуры данных потоков (`scratchpad`). Гарантировать через API, что потоки имеют доступ только к своим локальным аренам памяти.
- [x] **Hardened Gather:** Интегрировать обязательную проверку границ в операцию `MF_OP_GATHER`. При выходе индекса за пределы — немедленная активация `Kill Switch`.
