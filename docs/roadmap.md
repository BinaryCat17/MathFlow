...
## Phase 9: The "Cartridge" Model (Autonomous Packaging)
...
- [x] **Resource Templates in Binary:** Перенесено описание глобальных ресурсов и их флагов (`READONLY`, `PERSISTENT`) прямо в таблицу символов `mf_program`.
- [x] **Unified App Header:** Расширен `mf_bin_header`, добавлены параметры хоста (Window Title, Width, Height, VSync, NumThreads) прямо в файл программы.
- [x] **Zero-JSON Runtime (Partial):** Рантайм теперь может запускаться напрямую из `.bin/.mfc` без использования `.mfapp` или других JSON-файлов.
- [ ] **Binary Pipeline Bundling:** Научить формат хранить несколько программ (логика, физика, рендер) и описание их связей (Front/Back buffers) внутри одного файла.
- [ ] **Standalone Compiler CLI (`mfc`):** Создать утилиту, которая принимает JSON и все связанные ресурсы, проводит полную валидацию и «запекает» их в один финальный картридж.
- [ ] **Asset Embedding (The Blob):** Реализовать упаковку внешних ассетов (текстуры, шрифтов) прямо в хвост бинарного файла для создания 100% автономных приложений.

## Phase 10: Vectorization & High-Level DSL (Future)
